...
def process_table(args):
    ...
    output_filename = extract_data_to_csv(final_boxes, table_image, app, i)
...
def process_image(image, i):
    ...
    with ProcessPoolExecutor() as executor:
        output_filenames = executor.map(process_table, [(table, i) for i, table in enumerate(table_list)])
...
def main(file_path):
    ...
    if ext.lower() == '.pdf':
        ...
        for i, image in enumerate(images):
            ...
            page_output_filenames = process_image(image, i)
            for filename in page_output_filenames:
                output_filenames.append(filename)
        return output_filenames
    else:
        image = cv2.imread(file_path)
        output_filename = process_image(image, 0)
        return output_filename
